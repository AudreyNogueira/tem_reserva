...
public interface ReserveRepository extends JpaRepository<Reserve, Long> {
    ...
    @Query(value = "SELECT isnull(SUM(QTD_PESSOAS), 0) FROM TB_RESERVA R WHERE R.ID_RESTAURANTE = :idRestaurant AND PERIODO = :period AND CAST(DATA_RESERVA AS DATE) = CAST(:date AS DATE) AND CONFIRMADO = 1", nativeQuery = true)
    ...
    @Query(value = "SELECT isnull(SUM(R.QTD_PESSOAS), 0) AS currentPeople, PERIODO AS period FROM TB_RESERVA R WHERE R.ID_RESTAURANTE = :idRestaurant AND CAST(R.DATA_RESERVA AS DATE) = :date AND CONFIRMADO = 1 GROUP BY R.PERIODO", nativeQuery = true)
