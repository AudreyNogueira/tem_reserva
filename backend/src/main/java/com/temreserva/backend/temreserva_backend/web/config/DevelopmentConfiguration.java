...
public class DevelopmentConfiguration {
        ...
        public CommandLineRunner createDataForTesting(@Autowired SegmentRepository segmentRepository,
                        ...
                        @Autowired AddressRepository addressRepository, @Autowired ImageRepository imageRepository,
                        @Autowired ImageBusiness imageBusiness,
                        @Autowired MailTemplateRepository mailTemplateRepository) {
                return args -> {
                        System.out.println("Ambiente de desenvolvimento...");

                        mailTemplateRepository.save(MailTemplate.builder().description("reserve_success").html(template)
                                        .active(true).build());
                        ...
                        JSONArray arr = (JSONArray) jsonParser
                                        .parse(new FileReader("src/main/resources/restaurants-mock.json"));
                        ...
                        for (Object o : arr) {
                                ...
                                Restaurant restaurant = Restaurant.builder().credential(cred)
                                                .restaurantName(restaurantName).phoneNumber(phoneNumber).cnpj(cnpj)
                                                .description(description).cleaning(cleaning).payment(payment)
                                                .spacingOfTables(spacingOfTables).maxNumberOfPeople(maxNumberOfPeople)
                                                .build();
                                restaurant = restaurantRepository.save(restaurant);

                                Address addr = Address.builder().restaurant(restaurant).address(ad).cep(cep)
                                                .district(district).complement(complement).locality(locality)
                                                .restaurantNumber(restaurantNumber).zone(zone).uf(uf).build();
                                addressRepository.save(addr);

                                int imageNumber = (int) (Math.random() * ((8 - 1) + 1)) + 1;

                                MultipartFile file = new MockMultipartFile("Painting" + imageNumber + ".png",
                                                new FileInputStream(new File("src/main/resources/images/Painting"
                                                                + imageNumber + ".png")));
                                Image image = imageBusiness.buildImage(file, restaurant.getId(), true, true);
                                imageRepository.save(image);

                                for (int i = 0; i < 7; i++) {
                                        MultipartFile fileTest = new MockMultipartFile(
                                                        "Painting" + imageNumber + ".png",
                                                        new FileInputStream(
                                                                        new File("src/main/resources/images/Painting"
                                                                                        + imageNumber + ".png")));
                                        Image imageTest = imageBusiness.buildImage(fileTest, restaurant.getId(), false,
                                                        true);
